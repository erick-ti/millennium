...
class Command(BaseCommand):
    ...
    The public demo user signs in via ``POST /api/auth/demo-login/``, which ``login()``s
    ...
    **Create-only: an EXISTING account is left untouched** unless ``--repair`` is passed.
    Forcibly reactivating / de-privileging the row on every deploy
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if not options["repair"]:
            ...
            # non-seed account. demo-login will treat it as unavailable (404), which is safe,
            # not a hole. A WARNING (not an error) so the deploy one-shot still exits 0.
            self.stdout.write(
                self.style.WARNING(
