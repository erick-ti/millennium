...
# than a SyncRun, so it takes its own id in the shared advisory namespace beside the
# sync ids (1 = metadata, 2 = pricing, 3 = valuation). The lock serializes valuation
# passes so a manual `value_portfolios` can't race the scheduled task on the same-day
# get_or_create snapshot path.
...
def advisory_lock(lock_id: int) -> Iterator[bool]:
    ...
    Same-session re-entry would re-acquire (advisory locks are re-entrant per session),
    but each sync runs in its own worker process/connection, so contention is always
    cross-connection.
    """
