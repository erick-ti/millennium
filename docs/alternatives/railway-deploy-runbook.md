...
> **Reference artifact.** This Railway setup was evaluated and repo-prepped, but the
> project deployed to a self-hosted VPS instead. See `infra/hetzner/` and
> `ARCHITECTURE.md` (repo root) for the live deployment. This runbook is kept as a
> reference. Its environment matrix below predates a later change that dropped Redis
> in production, so it still lists `REDIS_URL`/Redis as required.
...
no comments, so **every "why" lives here**). The four topology decisions are: Railway
**cron** services (no always-on worker/beat), a **private-only** backend, **autodeploy**
on push (Wait-for-CI off), and the **generated** `*.up.railway.app` domain at launch.
...
- **4 cron services**: each builds the **backend** image and runs one management
  command on a schedule, then exits. They replace Celery worker+beat.
...
- **Wait for CI**: **OFF**. It waits on *all* check suites, so the one
...
- **frontend `healthcheckPath: /api/health/`** (NOT `/`): deliberately exercises the
  ...
  Slice-4 CI already proved the standalone
...
- **frontend `drainingSeconds: 150`**: the import path is TWO hops (browser → Next proxy →
  Django), so the frontend hop needs the same drain as the backend. With the
...
- **cron `restartPolicyType: NEVER` + deploy-during-run safety**: a clean exit-0 is normal
  ...
  **atomic** commands (`value_portfolios`, `run_alerts`, which write + run-record in one
...
|---|---|---|---|---|
| `DJANGO_SETTINGS_MODULE` | required | required | n/a | `config.settings.prod` (invariant 1 in ARCHITECTURE.md: fails closed if unset) |
| `DJANGO_SECRET_KEY` | required | required | n/a | strong random; same value across all backend-image services |
| `DJANGO_ALLOWED_HOSTS` | required | required | n/a | `backend.railway.internal,healthcheck.railway.app` |
| `DJANGO_CSRF_TRUSTED_ORIGINS` | required | required | n/a | exact `https://<app>.up.railway.app` (set after the domain exists, then redeploy) |
| `DATABASE_URL` | required | required | n/a | `${{Postgres.DATABASE_URL}}` (prefer the private host, **[verify live]**) |
| `REDIS_URL` | required | required | n/a | `${{Redis.REDIS_URL}}` |
| `CELERY_BROKER_URL` | required | required | n/a | `${{Redis.REDIS_URL}}`, idle (no worker) but fail-closed-required (invariant 2 in ARCHITECTURE.md) |
| `CELERY_RESULT_BACKEND` | required | required | n/a | `${{Redis.REDIS_URL}}` |
| `DJANGO_SECURE_SSL_REDIRECT` | `False` | `False` | n/a | private-only: plain-HTTP healthcheck must not 301 |
| `PORT` | `8000` | n/a | injected | pin backend web; cron exits; frontend uses injected |
| `DJANGO_SESSION_COOKIE_SAMESITE` | default `Lax` | default | n/a | exact `Lax`/`Strict`/`None` only, or boot fails |
| `DJANGO_CSRF_COOKIE_SAMESITE` | default `Lax` | default | n/a | same |
| `DJANGO_NUM_PROXIES` | default `0` | default | n/a | leave `0`; slice 8 sets the verified value after the XFF spoof test |
| `DJANGO_SECURE_HSTS_SECONDS` | default `0` | default | n/a | leave `0` until HTTPS proven on the final host (invariant 6 in ARCHITECTURE.md); ramp in slice 8+ |
| `DJANGO_SECURE_HSTS_INCLUDE_SUBDOMAINS` / `_PRELOAD` | default `False` | default | n/a | leave default |
| `SYNC_GUARD_METADATA_TOLERANCE` | default `0.02` | (ygoprodeck) | n/a | float in [0,1) |
| `SYNC_GUARD_PRICING_TOLERANCE` | default `0.10` | (tcgcsv) | n/a | float |
| `SYNC_GUARD_ARCHETYPE_TOLERANCE` | default `0.05` | (ygoprodeck) | n/a | float |
| `BACKEND_URL` | n/a | n/a | **build-time** | `http://backend.railway.internal:8000`, a Docker build ARG baked into `server.js`; a change needs a frontend **rebuild**, not a restart |
...
3. Verify: the `preDeployCommand` migrate ran; the healthcheck is green (it sends
   ...
   `/api/schema/` returns 403 anonymously in prod (invariant 7 in ARCHITECTURE.md). Create the superuser via
...
- Invariant 6 in ARCHITECTURE.md: HSTS off by default; ramp only after HTTPS is proven on the final host.
- Invariant 10 in ARCHITECTURE.md: `CSRF_TRUSTED_ORIGINS` must be the exact frontend origin (scheme incl.).
