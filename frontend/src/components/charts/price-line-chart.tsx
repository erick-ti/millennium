 ...
 * `ResponsiveContainer` to a fixed size under jsdom).
 *
 * Accessibility: Recharts emits an `aria-hidden` SVG, so the series
...
export function PriceLineChart({
  ...
  // id. useId carries colons (invalid in a CSS selector), so strip them; the SVG
      ...
      <div role="img" aria-label={caption} className="h-72 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 8, right: 16, bottom: 8, left: 8 }}>
            <defs>
              {/* Aged-gold area fill, 0.26 to 0 (the landing chart's treatment),
