...
/**
 * CSRF token injection for the same-origin `/api/*` proxy.
 *
 ...
 * CSRF-naive by design, so this proxy reads that cookie and copies it into
 * the `X-CSRFToken` request header before the request is forwarded upstream
 * to Django.
 *
 ...
 * URL-shape canonicalization, which Next runs *before* Proxy, which is why the
 ...
 *   - `request.cookies.get(...)` returns a `{ name, value }` object, so read
 *     `.value`, or you'd inject `[object Object]`.
 *
 * The `CSRF_TRUSTED_ORIGINS` Origin check is a separate CSRF gate handled by
 * Django settings (invariant 10 in ARCHITECTURE.md); this proxy only supplies
 * the token half.
 */
